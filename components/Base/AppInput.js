...
const Input = props => {
  ...
  const selectInput = () => {
    return (
      <>
        {label && <label className={`lbl-${name} mr-2 text-default-gray ${horizontalInput ? 'mt-2' : ''}`}>{label}{required && <span className="text-red-600 pl-1">*</span>}</label>}
        <div className={`select-wrapper relative bg-transparent border cursor-pointer flex justify-between items-center cursor-pointer relative text-default-gray  ${inputClass ? inputClass : ''}`} >
          <select 
            value={value} 
            onChange={onChange} 
            multiple={multiple} 
                ...
                {type !== 'textarea' ?
                  <div className="flex flex-row">
                    <input
                      ...
                      size={size}
                      className={`
                        ipt-${name} ${error ? 'border-1 border-default-gray' : ''} 
